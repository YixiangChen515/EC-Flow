...
def _create_observable_goal_envs(all_envs: EnvDict) -> EnvDict:
    ...
    for env_name, env_cls in all_envs.items():
        ...
        def initialize(env, seed=None, render_mode=None, camera_name=None):
            if seed is not None:
                st0 = np.random.get_state()
                np.random.seed(seed)
            super(type(env), env).__init__(render_mode=render_mode, camera_name=camera_name)
